...
var require_public_visibility = __commonJS({
  "supabase-src/shared/public-visibility.js"(exports, module) {
    ...
    async function getPublicVisibilityState({ edgeClient, userId }) {
      ...
      try {
        const { data, error } = await edgeClient.from("vibeusage_public_views").select("user_id,revoked_at,updated_at,share_token").eq("user_id", userId).maybeSingle();
    ...
    async function enablePublicVisibility({ edgeClient, userId, nowIso }) {
      ...
      if (existing?.user_id) {
        const { error: updateErr } = await table.update({ token_hash: tokenHash, share_token: shareToken, revoked_at: null, updated_at: updatedAt }).eq("user_id", userId);
...
export default require_vibeusage_public_visibility();
