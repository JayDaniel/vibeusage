...
## Verification
- `node scripts/copy-sync.cjs pull --dry-run` => diff shows local `dashboard.poster.title` entry vs origin/main (expected)
...
- `npm --prefix dashboard run build` => PASS (vite build, re-run after hiding day tab/range in screenshot mode)
