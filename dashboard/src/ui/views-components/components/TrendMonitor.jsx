...
export function TrendMonitor({
  ...
  return (
    <AsciiBox title={label} className={`w-full ${className}`} bodyClassName="flex flex-col gap-3">
      ...
      <div className="flex-1 relative overflow-hidden border border-[#E2E8F0] bg-[#FAFBFC] rounded-lg">
        {/* Clean subtle grid */}
        <div
        ...
        <div
          ref={axisRef}
          className="absolute right-0 top-0 bottom-0 flex flex-col justify-between py-1 px-1.5 text-[10px] text-[#94A3B8] font-medium pointer-events-none bg-white/80 border-l border-[#E2E8F0] w-10 text-right"
        >
        ...
        {hover ? (
          <>
            ...
            >
              <div
                className="absolute top-0 bottom-0 w-px bg-[#2563EB]/30"
                style={{ left: hover.x }}
              ></div>
              <div
                className="absolute w-2.5 h-2.5 rounded-full bg-[#2563EB] ring-2 ring-white"
                style={{ left: hover.x - 5, top: hover.y - 5 }}
              ></div>
            </div>
            <div
              className="absolute z-30 px-3 py-2 text-[11px] bg-white border border-[#E2E8F0] rounded-lg shadow-md text-[#1E293B] pointer-events-none"
              style={{
                left: Math.min(hover.x + 10, hover.rectWidth - hover.axisWidthPx - 120),
                top: Math.max(hover.y - 24, 6),
              }}
            >
              <div className="text-[#94A3B8] text-[10px]">{formatTooltipLabel(hover.label)}</div>
              {hover.missing ? (
                <div className="font-semibold">{copy("shared.status.unsynced")}</div>
              ) : (
      ...
      <div className="h-5 flex justify-between items-center px-1 text-[10px] text-[#94A3B8] font-medium border-t border-[#F1F5F9] pt-2">
