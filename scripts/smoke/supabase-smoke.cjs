...
async function main() {
  ...
  const beforeTotal = parseBigInt(
    beforeSummary?.totals?.total_tokens,
    ...
    JSON.stringify(
      {
        ...
        leaderboard: {
          ...
          week_page2: {
            ...
            entries: Array.isArray(leaderboardWeekPage2.entries)
              ? leaderboardWeekPage2.entries.length
              : null,
...
async function waitForSummaryTotalAtLeast({ baseUrl, accessToken, minTotal }) {
  ...
  throw new Error(
    `Usage summary did not reach expected total within timeout (last=${String(lastTotal)})`,
  );
}
