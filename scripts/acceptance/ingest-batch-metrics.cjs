...
function buildFetchStub() {
  ...
  async function handler(input, init = {}) {
    ...
    if (
      url.pathname === "/api/database/records/vibeusage_tracker_device_tokens" &&
      method === "GET"
    ) {
    ...
    if (
      url.pathname === "/api/database/records/vibeusage_tracker_ingest_batches" &&
      method === "POST"
    ) {
    ...
    if (
      url.pathname === "/api/database/rpc/vibeusage_touch_device_token_sync" &&
      method === "POST"
    ) {
