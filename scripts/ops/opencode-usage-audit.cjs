...
const DEFAULT_BASE_URL = "https://YOUR_PROJECT.supabase.co";
...
function resolveStorageDir(env) {
  ...
  const explicit =
    typeof env.OPENCODE_STORAGE_DIR === "string" ? env.OPENCODE_STORAGE_DIR.trim() : "";
...
async function runAuditCli(argv, deps = {}) {
  ...
  const baseUrl = args.baseUrl || env.VIBEUSAGE_SUPABASE_URL || DEFAULT_BASE_URL;
