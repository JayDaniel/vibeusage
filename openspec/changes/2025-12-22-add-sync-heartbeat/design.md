...
3. If `inserted === 0`, CLI sends a **throttled** heartbeat (min interval, e.g. 15 min).
...
### Non-negotiables
- Heartbeat frequency MUST be bounded (suggested minimum interval: 15 minutes).
...
4. **Throttling**: client heartbeat frequency enforced (configurable; default 15 min).
